...
export function predictWithModel(model: MLModel, features: ExoplanetFeatures): PredictionResult {
  ...
  const tempScore = features.koi_teq > 200 && features.koi_teq < 2000 ? 1 : 0;
  const depthScore = features.koi_depth > 50 ? 1 : 0;

  const totalScore = periodScore + radiusScore + snrScore + tempScore + depthScore;
...
export function generateTrainingData(numSamples: number = 100): ExoplanetFeatures[] {
  ...
  for (let i = 0; i < numSamples; i++) {
    data.push({
      ...
      koi_time0bk: Math.random() * 1000 + 100,
      koi_impact: Math.random() * 1.5,
      koi_duration: Math.random() * 10 + 0.5,
      koi_depth: Math.random() * 5000 + 10,
      koi_prad: Math.random() * 8 + 0.1,
      koi_teq: Math.random() * 1800 + 200,
      koi_insol: Math.random() * 1000 + 0.1,
      koi_model_snr: Math.random() * 100 + 5,
      koi_tce_plnt_num: Math.floor(Math.random() * 5) + 1,
      koi_steff: Math.random() * 3000 + 3500,
      koi_slogg: Math.random() * 2 + 3,
      koi_srad: Math.random() * 3 + 0.5,
      ra: Math.random() * 360,
      dec: Math.random() * 180 - 90,
      koi_kepmag: Math.random() * 10 + 10,
      koi_gmag: Math.random() * 10 + 10,
      koi_rmag: Math.random() * 10 + 10,
      koi_imag: Math.random() * 10 + 10,
      koi_zmag: Math.random() * 10 + 10,
    });
  }
